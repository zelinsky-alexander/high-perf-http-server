...
RESULT_DIR="$ROOT_DIR/results/python-asyncio/$(date -u +%Y%m%dT%H%M%SZ)"
...
usage() {
  cat <<'EOF'
Usage:
  benchmark-python-asyncio.sh [options] [url]
...
Examples:
  ...
  bash scripts/benchmark-python-asyncio.sh -c 10 -t 2 http://127.0.0.1:8080/plaintext
  bash scripts/benchmark-python-asyncio.sh -c 128 -t 4 -d 60s
EOF
}
