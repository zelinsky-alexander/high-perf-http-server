...
RESULT_DIR="$ROOT_DIR/results/go-nethttp/$(date -u +%Y%m%dT%H%M%SZ)"
...
usage() {
  cat <<'EOF'
Usage:
  benchmark-go-nethttp.sh [options] [url]
...
Examples:
  ./scripts/benchmark-go-nethttp.sh
  ./scripts/benchmark-go-nethttp.sh -c 128 -t 4 -d 60s http://127.0.0.1:8888/plaintext
EOF
}
...

{
  ...
  echo "go=$(go version)"
  echo "gomaxprocs=${GOMAXPROCS:-runtime-default}"
  lscpu
